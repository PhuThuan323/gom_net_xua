    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          message:
            "ID báo giá/hóa đơn không hợp lệ",
        });
      }
      ...
      if (!existing) {
        return res.status(404).json({
          success: false,
          message:
            "Không tìm thấy báo giá/hóa đơn",
        });
      }
      ...
      if (
        ...
        return res.status(409).json({
          success: false,
          message:
            "Báo giá này đã được xuất kho nên không thể sửa. " +
    ...
    try {
      ...
      if (
        ...
        return res.status(409).json({
          success: false,
          message:
            "Báo giá này đã được xuất kho nên không thể xóa. " +
      ...
      return res.json({
        ...
        message:
          `Đã xóa báo giá ${invoice.invoice_code}`,
      });
    } catch (error) {
      return sendError(
        res,
