...
async function createDebtTransaction(input: {
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      if (
        balanceAfter.lessThan(
          0
        )
      ) {
        throw new Error(
          `Số tiền trả/điều chỉnh vượt quá công nợ hiện tại (${currentBalance.toString()} đ)`
        );
      }
...
async function rebuildSupplierBalances(
  ...
  const rows =
    await tx.supplierDebt.findMany({
      where: {
        supplier_id:
          supplierId,
      },
  ...
  if (
    ...
    throw new Error(
      `Không thể sửa/xóa giao dịch vì tổng công nợ cuối cùng sẽ âm ${running
        .abs()
        .toString()} đ.`
    );
  }
...
function isLockedImportTransaction(
...
class DebtProviderController {
    ...
    try {
      ...
      return res.json({
        ...
        data: {
           ...
           */
          total_debt:
            totals.totalDebt.toString(),

          gross_debt:
            totals.grossDebt.toString(),

          total_payment:
            totals.totalPayment.toString(),

          total_adjustment:
            totals.totalAdjustment.toString(),

          total_balance:
            totals.currentBalance.toString(),

          overdue_balance:
            overdueBalance.toString(),
      ...
      const data =
        pageRows.map(
          (row) => ({
             ...
             */
            balance_after:
              row.calculated_balance_after.toString(),

            stored_balance_after:
  ...
  async createPayment(
    ...
    try {
      ...
      if (
        paymentAmount.lessThanOrEqualTo(
          0
        )
      ) {
        return res
      ...
      if (
        ...
        noteParts.push(
          `Hình thức thanh toán: ${payment_method.trim()}`
        );
      }
  ...
  async updateTransaction(
    ...
    try {
      ...
      const existing =
        await prisma.supplierDebt.findUnique({
          where: {
            id,
          },
      ...
      if (!existing) {
        return res
          ...
          .json({
            ...
            message:
              "Không tìm thấy giao dịch công nợ",
          });
      }

      if (
        isLockedImportTransaction(
      ...
      const result =
        await prisma.$transaction(
          async (tx) => {
            ...
            const updated =
              await tx.supplierDebt.findUnique({
                where: {
                  id,
                },
      ...
      if (!result.updated) {
        throw new Error(
          "Không thể đọc lại giao dịch sau khi cập nhật"
        );
      }
      ...
      return res.json({
        ...
        message:
          "Cập nhật giao dịch công nợ thành công",

        data: {
          ...result.updated,

          amount:
            result.updated.amount.toString(),

          balance_after:
            result.updated.balance_after.toString(),

          current_balance:
            result.currentBalance.toString(),
        },
      });
    } catch (error) {
      return errorResponse(
  ...
  async deleteTransaction(
    ...
    try {
      ...
      return res.json({
        ...
        message:
          "Xóa giao dịch công nợ thành công",
...
export default new DebtProviderController();
