...
async function getCurrentBalance(
  ...
  return latest?.balance_after ?? new Prisma.Decimal(0);
}
...
async function createDebtTransaction(input: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const balanceAfter =
      currentBalance.plus(balanceChange);

    if (balanceAfter.lessThan(0)) {
      throw new Error(
        `Số tiền trả vượt quá công nợ hiện tại (${currentBalance.toString()} đ)`
      );
    }
    ...
    return tx.supplierDebt.create({
      data: {
        ...
        transaction_date:
          input.transaction_date ?? new Date(),
      },
...
class DebtProviderController {
  ...
  async dashboard(req: Request, res: Response) {
    try {
      ...
      const totalAdjustment =
        adjustmentAggregate._sum.amount ??
        new Prisma.Decimal(0);
      ...
      for (const supplier of suppliers) {
        const balance =
          supplier.debts[0]?.balance_after ??
          new Prisma.Decimal(0);
  ...
  async transactions(
    ...
    try {
      ...
      const rawType =
        typeof req.query.transaction_type ===
        "string"
          ? req.query.transaction_type
          : undefined;

      let transactionType:
        DebtTransactionType | undefined;
